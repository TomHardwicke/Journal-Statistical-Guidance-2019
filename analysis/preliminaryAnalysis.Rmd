...
<br>
We have now completed data collection for the first phase of this project in which we extracted author-facing statistical guidance provided by the top-15 journals (by Impact Factor) in each of 22 scientific disciplines (i.e., total N = 330 journals). The preregistered study protocol is available [here](https://osf.io/cz9g3/). 

> *Operational definition of statistical guidance*: guidance related to the appropriate selection, implementation, reporting, and interpretation of statistical analyses
...
All data extraction was performed by a first coder (DS, MM, TB, MSH) and a second coder (TEH) with differences resolved through discussion.

*Note* Some publishers provided statistical guidance that was shared across their portfolio journals (specifically, 32 Nature journals, 12 Cell journals, 2 Frontiers journals) . Publisher guidance was coded by two team members (as above), but is represented in our data and analysis for each individual journal the guidance applies to. For example, the Nature guidance is inherited by 32 journals so appears 32 times in the dataset.

Below we present a preliminary analysis of the phase one data.
