...
library(ggforce) # helpful plotting functions
...
scale_fill_disciplines <- function(...){
...
scale_fill_domains <- function(...){
    ggplot2:::manual_scale(
        'fill', 
        values = setNames(c(hue_pal()(5),'#000000'), c(levels(factor(sum_tab_topics$domain[! sum_tab_topics$domain == 'OVERALL'])), 'OVERALL')), 
        ...
    )
}
```
...
sum_tab_domain <- sum_tab %>% 
  group_by(domain, variable) %>%
  summarise(n = sum(n), N = sum(N)) %>%
  mutate(prop = n/N, percent = n/N*100)

any_guidance_healthlife <- sum_tab_domain %>% filter(domain == 'HEALTH & LIFE SCIENCES', variable == 'guidance')
any_guidance_multi <- sum_tab_domain %>% filter(domain == 'MULTIDISCIPLINARY', variable == 'guidance')
any_guidance_social <- sum_tab_domain %>% filter(domain == 'SOCIAL SCIENCES', variable == 'guidance')
any_guidance_physical <- sum_tab_domain %>% filter(domain == 'PHYSICAL SCIENCES', variable == 'guidance')
any_guidance_formal <- sum_tab_domain %>% filter(domain == 'FORMAL SCIENCES', variable == 'guidance')
```

The frequency of journal author instructions providing statistical guidance varied considerably across domains (Health and Life Sciences: `r any_guidance_healthlife %>% pull(n)` of `r any_guidance_healthlife %>% pull(N)` journals, `r any_guidance_healthlife %>% pull(percent) %>% round()`%; Multidisciplinary: `r any_guidance_multi %>% pull(n)` of `r any_guidance_multi %>% pull(N)` journals, `r any_guidance_multi %>% pull(percent) %>% round()`%; Social Sciences: `r any_guidance_social %>% pull(n)` of `r any_guidance_social %>% pull(N)` journals, `r any_guidance_social %>% pull(percent) %>% round()`%; Physical Sciences: `r any_guidance_physical %>% pull(n)` of `r any_guidance_physical %>% pull(N)` journals, `r any_guidance_physical %>% pull(percent) %>% round()`; Formal Sciences: `r any_guidance_formal %>% pull(n)` of `r any_guidance_formal %>% pull(N)` journals, `r any_guidance_formal %>% pull(percent) %>% round()`.


Out of `r sum_tab %>% filter(esi_field == "OVERALL", variable == "guidance") %>% pull(N)` journals we found that `r sum_tab %>% filter(esi_field == "OVERALL", variable == "guidance") %>% pull(n)` (`r sum_tab %>% filter(esi_field == "OVERALL", variable == "guidance") %>% pull(percent)`%) offered statistical guidance (Figure 1A; for equivalent tabular data, see Supplementary Information C). This includes `r only_ext` journals which only referred to statistical guidance in external sources (reporting guidelines or academic papers) and `r sum_tab %>% filter(esi_field == "OVERALL", variable == "internal_guidance") %>% pull(n)` journals that provided their own guidance (of which `r crosstab_int_ext %>% filter(has_external_guidance == T, has_internal_guidance == T) %>% pull(n)` also referred to external sources). `r sum_tab %>% filter(esi_field == "OVERALL", variable == "internal_guidance_section") %>% pull(n)` (`r sum_tab %>% filter(esi_field == "OVERALL", variable == "internal_guidance_section") %>% pull(percent)`%) of `r sum_tab %>% filter(esi_field == "OVERALL", variable == "internal_guidance_section") %>% pull(N)` journals had a dedicated statistical guidance section in their author instructions (Figure 1B). Journals offered statistical guidance more often in Health and Life Sciences compared to other domains. Notably, all 15 Clinical Medicine journals offered some statistical guidance. In two domains (Computer Science and Mathematics), no journals offered any statistical guidance. Some journals shared the same publisher-level guidance, including `r publisher_nature_n` Nature journals, `r publisher_cell_n` Cell journals, and `r publisher_frontiers_n` Frontiers journals.
...
disciplineOrder <- c(disciplineOrder[! disciplineOrder == 'OVERALL'], 'OVERALL')

plot_any_dedicated_joint <- sum_tab_general %>%
  filter(variable %in% c('guidance', 'internal_guidance_section')) %>%
  mutate(esi_abbr = factor(esi_abbr, levels = disciplineOrder, ordered = T)) %>%
  ggplot(aes(x=esi_abbr, y = percent, fill = esi_abbr)) +
  ylim(0,100) +
...
# first make a data frame with overall data for each domain

domain_overalls <- sum_tab_general %>% 
  filter(esi_field != "OVERALL") %>% # remove previous overall summary
  group_by(domain, variable, var_display) %>%
  summarise(n = sum(n), N = sum(N)) %>%
  mutate(prop = n/N, percent = n/N*100, esi_abbr = paste(domain,'(ALL)'), esi_field = paste(domain,'(ALL)'), esi_field_with_abbr = paste(domain,'(ALL)'), priority = 2)

forPlot <- sum_tab_general %>% 
  filter(esi_field != "OVERALL") %>% # remove previous overall summary 
  mutate(priority = 1) %>%
  rbind(domain_overalls)

# set ordering of disciplines and domains
domainOrder <- domain_overalls %>%
  filter(variable == 'guidance') %>%
  arrange(desc(percent)) %>% 
  pull(domain)

axesOrder <- forPlot %>%
  filter(variable == 'guidance', esi_field != "MULTIDISCIPLINARY") %>%
  mutate(domain = factor(domain, levels = domainOrder, ordered = T)) %>%
  arrange(domain, priority, percent) %>% 
  pull(esi_field)

ready <- forPlot %>% 
  filter(esi_field != "MULTIDISCIPLINARY",
    variable %in% c('guidance', 'internal_guidance_section')) %>%
  mutate(esi_field = factor(esi_field, levels = axesOrder, ordered = T),
         domain = factor(domain, levels = domainOrder, ordered = T)) 
       

plot_any_dedicated_joint <- ready %>%
  ggplot(aes(y=esi_field, x = percent, fill = domain)) +
  xlim(0,100) +
  ggforce::facet_col(facets = vars(domain), 
                     scales = "free_y", 
                     space = "free") +
  geom_segment(aes(yend=esi_field, x=0, xend=percent), data =. %>% filter(variable == 'guidance')) +
  geom_point(size = 5, shape = 21, colour = 'black', data = . %>% filter(variable == 'guidance')) + 
  geom_point(size = 2, shape = 23, colour = 'black', data = . %>% filter(variable == 'internal_guidance_section')) + 
  geom_col(data = . %>% filter(variable == 'guidance', grepl('ALL', esi_field)), alpha = .2) + 
  xlab('journals (%)') +
  scale_fill_domains() +
  theme_apa() +
  theme(
...
n_topics <- d_all %>% filter(has_guidance == T, has_internal_guidance == T) %>%
  mutate(across(starts_with('has_'), ~replace_na(., FALSE) )) %>% # Recode NAs to FALSE
...
plot_all_topics <- sum_tab_topics %>% 
  ...
  ggplot(aes(x=topic_display, y = n)) +
    ylim(0,100) +
    geom_segment(aes(xend=topic_display, y=0, yend=n)) +
    geom_point(size = 5, shape = 21, fill = 'black', colour = 'black') + 
    ggtitle('B. Specific statistical guidance') +
    ylab('journals (n)') +
    theme_apa() +
    theme(
      panel.grid.major.x = element_line(),
...
```{r generaltopicsfignew, fig.path='figs/', dev=c('png', 'pdf'), fig.width = 8, fig.height = 9, fig.cap = "Percentage of journals offering some statistical guidance (Panel A circles), having a dedicated statistical guidance section (Panel A diamonds), or offering guidance on each of twenty prespecified statistical topics (Panel B).  Black circles represent data for all journals overall (N = 330 journals). Coloured circles and diamonds in Panel A represent individual scientific disciplines (n = 15 journals). Data is in descending order by percentage (after overall data in Panel A). CCV in Panel B refers to categorizing continuous outcomes. Discipline abbreviations: Agricultural Sciences (AGRI), Biology & Biochemistry (BIO), Chemistry (CHEM), Clinical Medicine (MED), Computer Science (COMSCI), Economics & Business (ECON), Engineering (ENGIN), Environment & Ecology (ECO), Geosciences (GEO), Immunology (IMMUN), Materials Science (MATSCI), Mathematics (MATH), Microbiology (MICBIO), Molecular Biology & Genetics (MOLBIO), Multidisciplinary (MULTI), Neuroscience & Behavior (NEURO), Pharmacology & Toxicology (PHARM), Physics (PHYS), Plant & Animal Science (PLANT), Psychiatry & Psychology (PSY), Social Sciences (SOCSCI), Space Science (SPACE)."}
plot_any_dedicated_joint & theme(plot.tag.position = c(0, 1),
        plot.tag = element_text(size = 14, hjust = 0, vjust = 0))
```
...
plotDomains <- function(thisTopic){
  ...
  ggplot(data = thisData, aes(x=esi_abbr, y = percent, fill = domain)) +
    ylim(0,100) +
    geom_segment(aes(xend=esi_abbr, y=0, yend=percent)) +
...
sum_tab_topics_domains <- sum_tab_topics %>% 
  group_by(domain, topic_display) %>% 
  summarise(n = sum(n), N = sum(N)) %>% 
  mutate(prop = n/N, percent = n/N*100)

# get order of topics in terms of most frequently mentioned
topics_ordered <- sum_tab_topics_domains %>%
  filter(domain == 'OVERALL') %>%
  arrange(percent) %>%
  pull(topic_display)
  
sum_tab_topics_domains <- sum_tab_topics_domains %>%
  mutate(topic_display = factor(topic_display, levels = topics_ordered, ordered = T))


customDomains <- c("HEALTH & LIFE SCIENCES"="HEALTH & LIFE SCIENCES", 
                      "MULTIDISCIPLINARY"="MULTI- DISCIPLINARY", 
                      "PHYSICAL SCIENCES"="PHYSICAL SCIENCES", 
                      "SOCIAL SCIENCES"="SOCIAL SCIENCES")

specific_fig_new <- sum_tab_topics_domains %>%
  filter(domain %notin% c('FORMAL SCIENCES','OVERALL')) %>% 
  ggplot(aes(y = topic_display, x = percent, fill = domain)) +
  facet_wrap(.~domain, nrow = 1, labeller = as_labeller(customDomains, multi_line = T, default=label_wrap_gen(10))) +
  #facet_wrap(.~domain, nrow = 1, labeller = label_wrap_gen(width = 2, multi_line = TRUE)) +
  xlim(0,60) +
  xlab('journals (%)') +
  geom_segment(aes(yend=topic_display, x=0, xend=percent)) +
...
```{r specifictopicsfignew, fig.path='figs/', dev=c('png', 'pdf'), fig.width = 8, fig.height = 7, fig.cap = "Caption here"}
specific_fig_new
```
...
```{r}
sum_tab_domain %>% filter(variable == 'external_guidance') %>% arrange(desc(percent))
```
